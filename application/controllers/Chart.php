...
class Chart extends CI_Controller {
	function grafik()
	{
		...
		$data['PieChartTitle']='Presentase Jumlah Penduduk Miskin Di D.I Yogyakarta Tahun 2014';

		//+1
		$chartData=file_get_contents('assets/jumpend14-15.json');
		$chartData=json_decode($chartData);
		$ress=array();
		foreach ($chartData as $row) {
			$datt=[$row->kab,(double)$row->thn14];
			array_push($ress, $datt);
		}
		 
		$data['PieChart']=json_encode($ress);
		$data['PieChartTitle11']='Presentase Jumlah Penduduk Di D.I Yogyakarta Tahun 2014';
